...
class ThesisAssetManager:
    def __init__(self, root: Union[str, Path] = ROOT_DIR_NAME, auto_create: bool = True):
        ...
        self.subdirs = {
            ...
            "latex": self.root / "latex",  # auto-generated LaTeX snippets
        }
        if auto_create:
            self._ensure_dirs()
    ...
    # ---------------- Public Save Methods -----------------
    def save_figure(self, fig: Any, section: str, name: str, version: str = "1.0", tags: Optional[List[str]] = None, description: str = "", source_code_reference: Optional[str] = None, formats: Optional[List[str]] = None, dpi: int = 300) -> List[AssetRecord]:
    ...
    def save_table(self, df: Any, section: str, name: str, version: str = "1.0", tags: Optional[List[str]] = None, description: str = "", source_code_reference: Optional[str] = None, include_json: bool = True) -> AssetRecord:
        if pd is None:
        ...
        # Also create a LaTeX snippet for quick inclusion
        try:
            latex_name = filename.replace('.csv', '.tex')
            latex_path = self.subdirs['latex'] / latex_name
            latex_table = self._dataframe_to_latex(df, caption=description or name.replace('_', ' ').title(), label=f"tab:{self._normalize(name)}")
    ...
    def save_data_snapshot(self, df: Any, section: str, name: str, version: str = "1.0", tags: Optional[List[str]] = None, description: str = "", source_code_reference: Optional[str] = None) -> AssetRecord:
        if pd is None:
    ...
    def _dataframe_to_latex(self, df: Any, caption: str, label: str, index: bool = False) -> str:
        if pd is None:
            raise RuntimeError("pandas not available")
        # Escape column headers
        df_escaped = df.copy()
        df_escaped.columns = [self._escape_latex(str(c)) for c in df_escaped.columns]
        if index:
            df_escaped.index = [self._escape_latex(str(i)) for i in df_escaped.index]
        # Use pandas built-in then adjust
        table_body = df_escaped.to_latex(index=index, escape=False)
        # Insert caption & label if not present
        lines = table_body.splitlines()
        # pandas produces a tabular environment only; wrap with table for full snippet
        wrapped = [r"\begin{table}[htbp]", r"  \centering"]
        wrapped += [f"  % Auto-generated by ThesisAssetManager"]
    ...
    def export_table_as_latex(self, section: str, name: str, version: str = "1.0") -> Optional[Path]:
        """Generate (or regenerate) a LaTeX snippet for a table already saved as CSV."""
        if pd is None:
            raise RuntimeError("pandas not available")
        csv_filename = self._build_filename(section, name, version, 'csv')
        csv_path = self.subdirs['tables'] / csv_filename
        if not csv_path.exists():
            return None
        df = pd.read_csv(csv_path)
        latex_name = csv_filename.replace('.csv', '.tex')
        latex_path = self.subdirs['latex'] / latex_name
        latex_table = self._dataframe_to_latex(df, caption=name.replace('_',' ').title(), label=f"tab:{self._normalize(name)}")
        latex_path.write_text(latex_table)
        return latex_path
